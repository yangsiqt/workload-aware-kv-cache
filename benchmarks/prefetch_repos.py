...
from benchmarks.workload_generator import _stratified_sample


def main() -> None:
    parser = argparse.ArgumentParser(description="Prefetch immutable SWE-bench repository snapshots")
    parser.add_argument("--config", type=Path, default=Path("configs/workloads.yaml"))
    parser.add_argument("--profile", default="pre_rental", help="Profile name from configs/workloads.yaml")
    ...
    manifest = {
        ...
        "profile": args.profile, "seed": config["seed"], "snapshots": artifacts,
    }
    path = data_root / "manifests" / f"repo-snapshots-{args.profile}.json"
    write_json(path, manifest)
    print(json.dumps({"path": str(path), "snapshots": len(artifacts),
                      "bytes": sum(row["bytes"] for row in artifacts)}, indent=2))
