...
async def run(args: argparse.Namespace) -> Path:
    ...
    run_id = args.run_id or f"{args.mode}-{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}-{uuid.uuid4().hex[:6]}"
    ...
    async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
        async def issue(item: WorkloadItem, offered: float) -> None:
            async with semaphore:
                result = await _request(
                    session, item, run_id=run_id, endpoint=config["endpoint"], model=config["model"],
                    tokenizer=tokenizer, route_policy=args.route_policy,
                    temperature=float(config.get("temperature", 0.0)), offered_at=offered,
                )
        ...
        if args.mode == "closed_loop":
            ...
            schedule: list[tuple[float, WorkloadItem]] = []
            delay = 0.0
            for item in sorted(items, key=lambda value: (value.session_id, value.turn_id)):
                delay += rng.expovariate(args.request_rate)
                schedule.append((delay, item))
            previous: dict[str, asyncio.Task[None]] = {}

            async def scheduled(delay_s: float, item: WorkloadItem, predecessor: asyncio.Task[None] | None) -> None:
            ...
            for delay_s, item in schedule:
                task = asyncio.create_task(scheduled(delay_s, item, previous.get(item.session_id)))
...
def main() -> None:
    ...
    parser.add_argument("--output-root", type=Path, default=Path("/root/workload-aware-kv-cache-data/runs"))
    parser.add_argument("--mode", choices=["closed_loop", "poisson"], default="closed_loop")
